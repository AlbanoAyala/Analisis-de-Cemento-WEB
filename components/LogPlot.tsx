...
const LogPlot: React.FC<LogPlotProps> = ({ logData, cblCurve, tocFound, tocRequested, depthCurveName, amplitudeRange, depthRange }) => {
  ...
  const binnedData = useMemo(() => {
    ...
    // Parse ranges strictly as numbers and ensure they are positive
    const dMin = Math.max(0, Number(depthRange.min));
    const dMax = Math.max(0, Number(depthRange.max));

    // Filter based on manual depth range to optimize performance
    // STRICT: depth must be > 0 and amplitude >= 0
    const validData = logData.filter(d => 
        d != null && 
        typeof d[depthCurveName] === 'number' && !isNaN(d[depthCurveName]) && d[depthCurveName] > 0 &&
        d[depthCurveName] >= dMin && d[depthCurveName] <= dMax &&
        typeof d[cblCurve] === 'number' && !isNaN(d[cblCurve]) && d[cblCurve] >= 0
    );
  ...
  if (binnedData.length === 0) {
    return (
      <div className="h-full flex items-center justify-center bg-dark-card">
        <p className="text-dark-muted text-xs uppercase tracking-wider">Sin datos válidos en este rango</p>
      </div>
    );
  }
        ...
        <div style={{ height: `${chartHeight}px`, width: '100%', minHeight: '100%' }}>
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <XAxis 
                type="number"
                domain={[Math.max(0, amplitudeRange.min), amplitudeRange.max]}
