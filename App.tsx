...
const Header = ({ toggleSidebar, wellName }: { toggleSidebar: () => void, wellName?: string }) => (
    <header className="h-16 bg-dark-card border-b border-dark-border flex items-center justify-between px-4 shadow-md shrink-0 z-20">
        ...
        {/* CGC Logo in Header - Height adjusted to prevent clipping */}
        <div className="flex flex-col w-24 shadow-lg origin-right mr-2">
            <div className="bg-black text-white text-xl font-black flex items-center justify-center py-1.5 tracking-[0.2em] leading-none">
                CGC
            </div>
            <div className="h-2 bg-[#009b9b] w-full shrink-0"></div>
        </div>
    </header>
);
...
const App: React.FC = () => {
    ...
    const parsedAmplitudeRange = useMemo(() => {
        ...
        return {
            min: isNaN(min) ? 0 : min,
            max: isNaN(max) || max <= min ? (isNaN(min) ? 100 : min + 10) : max,
        };
    }, [amplitudeRange]);
    ...
    const parsedDepthRange = useMemo(() => {
        ...
        return {
            min: isNaN(min) ? 0 : min,
            max: isNaN(max) || max <= min ? (isNaN(min) ? 3000 : min + 100) : max,
        };
    }, [depthRange]);
